...
【動画編集の原則（Lesson 2/4/5に基づく）】

1. **テンポとリズム（Lesson 5）**
   - 各クリップは30-90秒が理想的（7秒ルール：同じ画面が7秒以上続くと飽きられる）
   - 視聴維持率40%超を目指す設計
   - 冒頭は問題提起型・結果先出し型で引き込む
   - 3幕構成（導入→展開→結末）で緩急をつける

2. **カット編集技術（Lesson 2）**
   - **ジャンプカット**: 同じアングルで不要部分を中抜き、テンポアップ
   - **マッチカット**: 形・動き・音の共通要素で場面転換をスムーズに
   - **対比カット**: 対照的な2シーンでギャップを作り印象を強める
   - **時間経過/メタファー**: 長時間を短く表現、象徴的な表現で感情を増幅

3. **見どころの選定基準（実務KPI）**
   - コメントが集中している箇所（視聴者の反応が高い）
   - 視聴維持率が急上昇する箇所（30秒・50%・90%地点の落ち込みなし）
   - 盛り上がりのある展開（エンタメ性、情報の価値）
   - ストーリー性のあるシーン（共感・参加を促す）

4. **視認性と読みやすさ（Lesson 4）**
   - 重要なセリフやシーンを含める（字幕で補強できる箇所）
   - 文脈がわかる箇所を選ぶ（途中から始まらない）
   - 1秒=4文字のペースで字幕が追える速度

【重要な制約】
- **動画の開始2分間（0-120秒）は絶対に除外**（視聴維持率が必然的に高いため）
- 見どころ同士が30秒以上重複しないこと（重複は視聴者を飽きさせる）
- 合計時間は約10分（600秒）を目標
- 長すぎるクリップ（90秒超）は避ける
...
【条件】
1. 各見どころは30秒〜90秒の長さ（7秒ルールを考慮）
2. 見どころ同士が30秒以上重複しないこと
3. 動画全体から均等に選ぶこと（開始2分間を除く）
4. スコアが高い順にソートすること
5. コメントや視聴維持率の急上昇箇所を優先
6. **開始120秒（2分間）は絶対に含めないこと**
7. 対比カット・マッチカットが可能な箇所を優先
